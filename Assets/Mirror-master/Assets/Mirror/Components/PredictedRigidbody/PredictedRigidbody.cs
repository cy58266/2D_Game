    ...
    public class PredictedRigidbody : NetworkBehaviour
    {
        ...
        protected virtual void SmoothFollowPhysicsCopy()
        {
            ...
            */
            ...
            GetPositionAndRotation(tf, out Vector3 currentPosition, out Quaternion currentRotation); // faster than tf.position + tf.rotation
            GetPositionAndRotation(predictedRigidbodyTransform, out Vector3 physicsPosition, out Quaternion physicsRotation); // faster than Rigidbody .position and .rotation
        ...
        void FixedUpdate()
        {
            ...
            if (onlyRecordChanges)
            {
                ...
                GetPositionAndRotation(tf, out Vector3 position, out Quaternion rotation);
                // clean & simple:
        ...
        void RecordState()
        {
            ...
            GetPositionAndRotation(tf, out Vector3 currentPosition, out Quaternion currentRotation); // faster than accessing .position + .rotation manually
#if UNITY_6000_0_OR_NEWER
        ...
        void OnReceivedState(double timestamp, RigidbodyState state)//, bool sleeping)
        {
            ...
            GetPositionAndRotation(predictedRigidbodyTransform, out Vector3 physicsPosition, out Quaternion physicsRotation);
        ...
        public override void OnSerialize(NetworkWriter writer, bool initialState)
        {
            ...
            // FAST VERSION: this shows in profiler a lot, so cache EVERYTHING!
            GetPositionAndRotation(tf, out Vector3 position, out Quaternion rotation);  // faster than tf.position + tf.rotation. server's rigidbody is on the same transform.
